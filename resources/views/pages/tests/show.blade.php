          ...
          <div class="flex flex-col md:flex-row gap-8">
            <div class="md:w-1/2">
              <div class="relative w-full h-72 md:h-96 lg:h-[480px] overflow-hidden rounded-lg">
                @if ($mainImage)
                    <img 
                        src="{{ rtrim(config('services.api.storage_url'), '/') }}/{{ $mainImage }}" 
                        alt="{{ $test['name'] }}" 
                ...
                <div class="absolute top-4 right-4">
                  @if ($test['is_active']) <span class="bg-green-500 text-white px-3 py-1 rounded-full text-sm font-medium">Tersedia</span> @else <span class="bg-red-500 text-white px-3 py-1 rounded-full text-sm font-medium">Tidak Tersedia</span> @endif
                </div>
                <div class="absolute bottom-4 left-4"><span class="bg-black/70 text-white px-3 py-1 rounded-full text-sm font-medium">Rp {{ number_format($test['price'], 0, ',', '.') }}</span></div>
              </div>
              ...
              @if (count($imagePaths) > 1)
                <div class="flex mt-4 gap-2 overflow-x-auto pb-2">
                  @foreach($imagePaths as $index => $path)
                    <div
                      class="relative w-20 h-20 flex-shrink-0 rounded-md overflow-hidden border-2 
                             {{ $index == 0 ? 'border-blue-300' : 'border-gray-300' }}"
                    >
                      <img src="{{ rtrim(config('services.api.storage_url'), '/') }}/{{ $path }}" alt="{{ $test['name'] }} - thumbnail {{ $index + 1 }}" class="w-full h-full object-cover" />
                    </div>
                  @endforeach
                </div>
              @endif
            ...
            <div class="md:w-1/2">
              <a href="{{ route('tests.index') }}" class="text-blue-300 hover:text-white flex items-center text-sm mb-4"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg> Kembali ke Daftar Pengujian</a>
              <h1 class="text-3xl font-bold mb-2">{{ $test['name'] }}</h1>
              <div class="mb-4"><span class="inline-block bg-blue-600 text-white px-3 py-1 rounded-full text-xs font-medium">{{ $test['category']['name'] }}</span></div>
